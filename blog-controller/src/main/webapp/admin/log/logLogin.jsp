  ...
  <head>
	...
	function deleteLog() {
		...
		$.messager.confirm("系统提示", "您确认要删除这<font color=red>" + selectedRows.length
			...
			if (r) {
				$.post("${cxt}/admin/log/login/delete.shtml", {ids : ids}, function(result) {
					if (result.success) {
	...
	function manualBackup(){
		$.messager.confirm("系统提示", "备份之后会将原来的日志删除。确定吗？", function(r) {
			if (r) {
				$.post("${cxt}/admin/log/login/backup.shtml", function(result) {
					if (result.success) {
	...
	function downloadLogBus(){
		...
		$('#backupTable').datagrid({       
		    	...
		    	url:"${cxt}/admin/backup/list.shtml?backupType=登录日志",//url调用Action方法  
	...
	// 下载Log4j文件
	function exportData(){
		location.href="${cxt}/admin/log/login/exportData.shtml";
	}
</script>
    </head>
...
<table  id="dg" class="easyui-datagrid" fitColumns="true"  nowrap="false"
   	    pagination="true" rownumbers="true" url="${cxt}/admin/log/login/list.shtml" fit="true" toolbar="#tb">
        <thead>
            	<tr>
...
<div id="tb" >
	...
	<div>
		...
		<a clazz="easyui-linkbutton" onClick="exportData()" class="easyui-linkbutton" iconCls="icon-edit" >导出数据</a>
	</div>
	<div class="updownInterval"> </div>
	<div>
...
<table  class="easyui-datagrid" id="backupTable" >
	...
	<script type="text/javascript">
    		function formatDownload(val,row){
				return "<a href='${cxt}/admin/log/login/downloadBackup.shtml?fileName="+val+"'>"+val+"</a>";
        	}
	</script>
