    ...
    <script type="text/javascript">
        ...
        function deleteLog() {
            ...
            $.messager.confirm("系统提示", "您确认要删除这<font color=red>" + selectedRows.length
                ...
                if (r) {
                    $.post("${cxt}/admin/log/operation/delete.shtml", {ids: ids}, function (result) {
                        result = eval("(" + result + ")");
                        if (result.success) {
        ...
        function manualBackup() {
            $.messager.confirm("系统提示", "备份之后会将原来的日志删除。确定吗？", function (r) {
                if (r) {
                    $.post("${cxt}/admin/log/operation/backup.shtml", function (result) {
                        if (result.success) {
        ...
        function downloadLogBus() {
            ...
            $('#backupTable').datagrid({
                ...
                url: "${cxt}/admin/backup/list.shtml?backupType=操作日志",//url调用Action方法
        ...
        function downloadLog() {
            location.href = "${cxt}/admin/log/operation/downloadLog.shtml";
        }
...
<table id="dg" class="easyui-datagrid" fitColumns="" nowrap="false"
       pagination="true" rownumbers="true" url="${cxt}/admin/log/operation/list.shtml" fit="true" toolbar="#tb">
    <thead>
    <tr>
        ...
        <th field="parameter" align="center">方法参数</th>
...
<div id="tb">
    ...
    <div class="updownInterval"></div>
    <div>
        &nbsp;操作人： &nbsp;<input type="text" name="s_userName" id="s_userName" size="20"
                                onkeydown="if(event.keyCode==13) searchLog()"/>
        &nbsp;操作类型：&nbsp;<input type="text" name="s_operation" id="s_operation" size="20"
                                onkeydown="if(event.keyCode==13) searchLog()"/>
        &nbsp;所属模块：&nbsp;<input type="text" name="s_module" id="s_module" size="20"
                                onkeydown="if(event.keyCode==13) searchLog()"/>
...
<script type="text/javascript">
    function formatDownload(val, row) {
        return "<a href='${cxt}/admin/log/operation/downloadBackup.shtml?fileName=" + val + "'>" + val + "</a>";
    }
</script>
