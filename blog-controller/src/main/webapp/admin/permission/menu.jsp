...
<%@ taglib uri="www.ttitcn.cn" prefix="privilege"  %>
...
<script type="text/javascript">
	...
	$(function(){
		//  加载树形表格菜单
		$('#treeGrid').treegrid({
			url:'${cxt}/admin/menu/list.shtml?parentId=-1',
			onLoadSuccess:function(){
				//全部展开
				 $("#treeGrid").treegrid('expandAll');
			},
			onBeforeExpand:function(node){
				$('#treeGrid').treegrid('options').url = "${cxt}/admin/menu/list.shtml?parentId=" + node.id;
			},
	...
	function openMenuAddDialog(){
		...
		url="${cxt}/admin/menu/save.shtml?parentId="+node.id;
		nodeId=node.id;
	}
    var  nodeId;
	// 菜单添加或修改的保存
	...
	function openMenuUpdateDialog(){
		...
		*/
		...
		url="${cxt}/admin/menu/modify.shtml?menuId="+node.id;
	}
	...
	function deleteMenu(){
		...
		$.messager.confirm("系统提示","您确认要删除这个菜单节点吗?",function(r){
			if(r){
				$.post("${cxt}/admin/menu/delete.shtml",{menuId:node.id,parentId:parentNode.id},function(result){
					if(result.success){
	...
	function openOperationDialog(){
		...
		var url = "${cxt}/admin/operation/list.shtml?menuId="+row.id;
	...
	function openOperationAddDialog(){
		...
		url2="${cxt}/admin/operation/save.shtml";
	...
	function openOperationUpdateDialog(){
		...
		url2="${cxt}/admin/operation/modify.shtml?operationId="+row.operationId;
	}
	...
	function deleteOperation(){
		...
		$.messager.confirm("系统提示","您确认要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("${cxt}/admin/operation/delete.shtml",{ids:ids},function(result){
					if(result.success){
...
<div id="tb">
	<div>
		<privilege:operation operationId="菜单管理:添加" clazz="easyui-linkbutton" onClick="openMenuAddDialog()" name="添加"  iconCls="icon-add" ></privilege:operation>
		<privilege:operation operationId="菜单管理:修改" clazz="easyui-linkbutton" onClick="openMenuUpdateDialog()" name="修改"  iconCls="icon-edit" ></privilege:operation>
		<privilege:operation operationId="菜单管理:删除" clazz="easyui-linkbutton" onClick="deleteMenu()" name="删除"  iconCls="icon-remove" ></privilege:operation>
		<privilege:operation operationId="菜单管理:按钮管理" clazz="easyui-linkbutton" onClick="openOperationDialog()" name="按钮管理"  iconCls="icon-edit" ></privilege:operation>
	</div>
</div>
