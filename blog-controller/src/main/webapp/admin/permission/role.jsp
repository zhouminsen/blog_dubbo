...
<script type="text/javascript" src="${cxt}/static/jquery-easyui-1.3.3/locale/easyui-lang-zh_CN.js"></script>
	...
	function deleteRole(){
		...
		$.messager.confirm("系统提示","您确认要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("${cxt}/admin/role/delete.shtml",{ids:ids},function(result){
					if(result.success){
	...
	function openRoleUpdateDialog(){
		...
		url="${cxt}/admin/role/modify.shtml?roleId="+row.roleId;
	}
	...
	function openMenuDialog(){
		...
		url="${cxt}/admin/menu/chooseMenu.shtml?parentId=-1&roleId="+roleId;
		// 加载所有权限的树
		$("#tree").tree({
			lines:true,
			url:url,
	...
	function saveMenu(){
		...
		$.post("${cxt}/admin/role/modifyRoleMenu.shtml",{menuIds:menuIds,roleId:roleId},function(result){
			if(result.success){
...
<table id="dg" class="easyui-datagrid" fitColumns="true" 
    pagination="true" rownumbers="true" url="${cxt}/admin/role/list.shtml" fit="true" toolbar="#tb">
    <thead>
    	<tr>
...
<div id="tb">
	...
	<div>
		<privilege:operation operationId="角色管理:添加" clazz="easyui-linkbutton" onClick="openRoleAddDialog()" name="添加"  iconCls="icon-add" ></privilege:operation>
		<privilege:operation operationId="角色管理:修改" clazz="easyui-linkbutton" onClick="openRoleUpdateDialog()" name="修改"  iconCls="icon-edit" ></privilege:operation>
		<privilege:operation operationId="角色管理:删除" clazz="easyui-linkbutton" onClick="deleteRole()" name="删除"  iconCls="icon-remove" ></privilege:operation>
		<privilege:operation operationId="角色管理:授权" clazz="easyui-linkbutton" onClick="openMenuDialog()" name="授权"  iconCls="icon-edit" ></privilege:operation>
	</div>
	<div class="updownInterval"> </div>
	<div>
