...
$(function(){
	...
	$("#dg").datagrid({
		url:"${cxt}/admin/blog/type/list.shtml?v="+Math.random(),
		//数据表格的数据
		title:"博客类别管理",
		...
		toolbar:[
		   ...
		   {text:"删除",iconCls:"icon-remove",handler:function(){
		  	  if (flag) {
				   ...
				   $.post("${cxt}/admin/blog/type/delete.shtml?v="+Math.random(),{"ids":ids},function(result){
					   if (result) {
		...
		onAfterEdit:function(index,row){
			//直接把数据更新到服务器
			if(method=="save"){
				$.post("${cxt}/admin/blog/type/save.shtml?v="+Math.random(),row,function(result){
				...
				$.ajax({
					url:"${cxt}/admin/blog/type/modify.shtml?v="+Math.random(),
					data:row,
					dataType:"json",
					type:"post",
