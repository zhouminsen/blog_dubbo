...
<script type="text/javascript">
	...
	function formatTitle(val,row){
		return "<a target='_blank' href='${cxt}/blog/detail/"+row.id+".shtml'>"+val+"</a>";
	}
	
	function releaseOrBackout(id,state){
		$.post("${cxt}/admin/blog/releaseOrBackout.shtml",{id:id,state:state},function(result){
			if (result.success) {
	...
	function deleteBlog(){
		...
		if(selectedRows.length==0){
		 ...
		 $.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
				if(r){
					/* //普通http请求
					location.href="${cxt}/admin/blog/delete.shtml?ids="+ids; */
					
		 		/****************post 有error方法**********************/
				/*  $.post("${cxt}/admin/blog/delete.shtml",{ids:ids},function(result){
				...
				$.ajax({
					type : "post",//指定请求方式
					url : "${cxt}/admin/blog/delete.shtml", //请求路径
					data : {ids:ids},	//请求参数
	...
	function restore(){
		...
		$.post("${cxt}/admin/blog/restore.shtml",{ids:ids},function(result){
				if (result) {
	...
	function openBlogModifyTab() {
		...
		var node={
				text:"修改博客",
				attributes:{
							 "url":"/admin/blog/modifyUI.shtml?id="+ row.id+""
							},
...
<table id="dg" title="博客管理" class="easyui-datagrid"
   ...
   url="${cxt}/admin/blog/list.shtml" fit="true" toolbar="#tb">
   <thead>
   	<tr>
 ...
 <div id="tb">
 	<div>
 		<shiro:hasPermission name="博文管理:修改">
