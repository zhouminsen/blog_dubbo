...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#tree").tree({
			...
			<%--url : '${cxt}/admin/menu/getMenuTree.shtml?parentId=-1',--%>
			<%--onLoadSuccess : function() {--%>
				<%--//全部展开--%>
				 <%--$("#tree").tree('expandAll');--%>
			<%--},--%>
			onBeforeExpand:function(node){
				$('#tree').tree('options').url = "${cxt}/admin/menu/list.shtml?parentId=" + node.id;
			},
	...
	function openPasswordModifyDialog() {
		...
		url = "${cxt}/admin/user/modifyPassword.shtml?id=${currentUser.id}";
	}
	...
	function logout() {
		$.messager.confirm("系统提示", "您确定要退出系统吗？", function(r) {
			if (r) {
				window.location.href = '${cxt}/admin/logout.shtml';
			}
		});
	}

	function refreshSystem() {
		$.post("${cxt}/admin/system/refreshSystem.shtml", {}, function(result) {
			if (result.success) {
